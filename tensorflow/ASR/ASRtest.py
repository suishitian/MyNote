...
def speech_to_text_network(n_dim=128, n_blocks=3):
    ...
    def residual_block(input_sensor, size, rate):
        conv_filter = aconv1d_layer(input_tensor=input_sensor, size=size, rate=rate, activation='tanh', scale=0.03,
                                    bias=False)
        conv_gate = aconv1d_layer(input_tensor=input_sensor, size=size, rate=rate, activation='sigmoid', scale=0.03,
                                  bias=False)
...
def train_speech_to_text_network(wav_max_len):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(16):
            ...
            if epoch % 5 == 0:
                saver.save(sess, r'/speech/module', global_step=epoch)
